...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: seoMeta({
      title: `Contact ${site.shortName} - Home Nursing in Katpadi, Vellore`,
      description:
        "Contact Care Touch Nursing & Home Care Agency in Katpadi, Vellore. Call 88255 00905 or 94427 56140 for 24/7 home nursing, elder care, doctor visits and ambulance support.",
      path: "/contact",
    }),
      ...
      <section className="bg-gradient-hero">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-16 lg:py-20 text-center">
          ...
          <p className="mt-4 max-w-xl mx-auto text-muted-foreground">
            Call us directly or send a message - our coordinator will respond promptly.
          </p>
        </div>
      </section>
          ...
          {[
            {
              icon: Phone,
              title: "Phone",
              lines: ["88255 00905", "94427 56140"],
              phoneLinks: ["tel:8825500905", "tel:9442756140"],
            },
            { icon: WhatsAppIcon, title: "WhatsApp", lines: ["88255 00905"], href: whatsappHref },
            {
              icon: Mail,
              title: "Email",
              lines: ["caretouchnursinghomecare@gmail.com"],
              href: "mailto:caretouchnursinghomecare@gmail.com",
            },
            {
              icon: MapPin,
              title: "Address",
              lines: [
                "No 91, KR Complex, Vallimalai Rd,",
                "opposite to vinayagar temple, Kumarappan Nagar,",
                "KRS Nagar, Katpadi, Vellore, Tamil Nadu 632007",
              ],
              href: mapsHref,
            },
            ...
            const Body = (
              <div className="flex gap-4 rounded-2xl border border-border/60 bg-card p-5 hover:border-brand/40 hover:shadow-card transition">
                <div
                  className={`h-11 w-11 rounded-xl flex items-center justify-center shrink-0 ${c.title === "WhatsApp" ? "bg-[#25D366] text-white" : "bg-brand-soft text-brand-deep"}`}
                >
            ...
            return "href" in c ? (
              <a
                key={c.title}
                href={c.href}
                target={c.title === "WhatsApp" || c.title === "Address" ? "_blank" : undefined}
                rel={c.title === "WhatsApp" || c.title === "Address" ? "noreferrer" : undefined}
                className="block"
              >
                {Body}
              </a>
        ...
        >
          ...
          <div className="mt-6 grid gap-4">
            <input
              ...
              className="rounded-xl border border-input bg-background px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand/30"
            />
            <input
              required
              name="phone"
            ...
            <select
              ...
              name="service"
              defaultValue=""
              className="rounded-xl border border-input bg-background px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-brand/30"
            >
